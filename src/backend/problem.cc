...
bool Problem::SolvebyDogleg(int iterations) {
    ...
    TicToc t_solve;
       ...
       MakeHessian();
       //计算chi2
       ComputeTrustRegionInitDL();
    //parameters
    int iter = 0;
    double v = 2.;
    bool found = false;
    //step1 初始信任区域半径sΔk
    double radius = 1e4;   //initial trust region radius 
    double rou;     //gain ratio 增益比
    ...
    {
       std::cout << "iter: " << iter << " , currentChi= " << currentChi_ << " , radius= " << radius << std::endl;
        ...
        while(inner_iter++ < inner_iterations)
        {
            ...
            current_squared_error = currentChi_;
            g_ = b_;
       
            //step3 分别根据GaussNewton法和最快下降法计算hgn和hsd,然后计算最快下降法的迭代步长α
            //compute step for sdd 
            double alpha = g_.squaredNorm() / (Hessian_*g_).dot(g_);
            //steepest descent step at current estimation point 最速下降法step
            VecX h_sd = alpha * g_;
            // SolveLinearSystem();
            //gauss-newton step at current estimation point 高斯牛顿法step
            VecX h_gn = Hessian_.ldlt().solve(b_);
            //dogleg step
            VecX h_dl;
            //step4 根据hgn、hsd和trust region半径Δk，来计算Dog-Leg步进值hdl
            //compute dog leg step for 计算dogleg的步进值
            ...
            //step5 计算增益比ρ,根据增益比大小更新trust region半径Δk
            //compute gain ratio
            double new_squared_error = 0.;
            new_squared_error = tempChi;
            ...
            //根据增益比大小更新trust region半径Δk
            //a valid decreasing step
            if(rou > 0 && isfinite(new_squared_error)){
...
void Problem::ComputeTrustRegionInitDL()
{
    currentChi_ = 0.0;
